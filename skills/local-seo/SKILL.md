...
- Lead with the useful info; work 1–2 local keyword phrases in naturally (city + make/service).
- Use GBP's CTA buttons (Book / Call / Learn more) rather than "link in bio."
- Keep within GBP length (~1,500 chars; front-load value).
- Each post = one clear topic (offer, tip, update). Tie to a real service or event.

## NAP consistency (hard)
- Business **N**ame, **A**ddress, **P**hone must match approved data exactly and the **right location**. Never guess an address, phone, hours, or the Hollywood opening date — escalate if unknown (`brand-voice` claims rules).

## Hashtags vs keywords (don't confuse platforms)
