...
`requiredCaveats` and `forbiddenClaims` are passed to stages 3–5 as binding restrictions and to the critic as its yardstick, so write each one precisely and only where the evidence warrants it. Neither list is a filter anything runs, so a claim you leave out of `forbiddenClaims` is not thereby permitted. Nothing is permitted except what you bound to a fact id.
