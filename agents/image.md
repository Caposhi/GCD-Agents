...
5. **Never show the vehicle's license plate — this is a hard requirement, not a style preference.** A photoreal hero shot routinely renders believable-looking (or borderline-readable) texture on a plate even when told to leave it "blank," and that has repeatedly failed publication QC. Do not compose a shot that puts a plate area in frame and then ask for it to be blank — that keeps failing. Instead choose a composition that excludes the plate by construction: a front 3/4 angle cropped above the bumper/plate line, a rear 3/4 angle, or a tight shot on the grille/headlight/front fascia that never reaches down to where a plate would be. (Runtime also sends a negative prompt discouraging plate text as backup, but the composition choice is what actually prevents it.)
