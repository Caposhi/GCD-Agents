...
`content_evidence` and `content_evidence_relations` are the durable substrate for Phase 0B reasoning. **Migration 006 is merged to `main` (`44d7336…`) and was applied to production on 2026-08-28 at `15:24:18Z` in approximately 53 ms — operator-verified, not independently verified in an engineering session.** It was applied exactly once, by the API pre-deploy runner, and both tables were confirmed empty. The rollout then **stopped at step 6** under stop conditions S8/S18 because the runbook's index count was wrong, so the **worker and scheduler remain at `a6a4316…`** — a mixed version that is a proven-compatible safe pause state. See the [Phase 0B.0 rollout runbook](ROLLOUT_PHASE_0B0.md).
