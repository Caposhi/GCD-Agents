...
## Content evidence sync (Phase 0B.0 — merged, not deployed)
...
**Do not run this against production in an unauthorized session.** Migration 006 is merged but **not applied to production**, so there is currently nothing there to sync into. A first production sync is a separately authorized operation that follows the migration rollout — it is explicitly **not** part of it. See the [Phase 0B.0 rollout runbook](ROLLOUT_PHASE_0B0.md).
