...
4. Back up an existing database before deployment. The API predeploy runs `npm run migrate` automatically. For migration 005, stop the legacy scheduler/worker as a hard gate because that publication binary ignores `revoked_at`; keep it stopped and drain or reject every existing pending approval first. Legacy pending/approved rows are deliberately revoked, the plaintext-token column is cleared/constrained to NULL, and every bound Phase0A subject requires nonnull equal canonical/legacy copies. Unbound historical rows are not backfilled into valid approvals. Review the actual approval/media table volume, digest backfill/scan/locks, and permanent trigger that blocks media id/MIME/bytes/digest changes plus every delete. Migration 005 fails after 10 seconds waiting for a lock or five minutes in one statement; investigate rather than loop a timed-out predeploy. Ensure exactly one migration runner/process invokes the migration; do not permit concurrent predeploy runners. Apply migration 005 before starting only the compatible API/worker/scheduler; do not backfill or preserve old approval links, and require fresh review after code is live.
5. Keep scheduler and worker suspended until account identity, canonical approved facts/CTA URLs, approval channel, public HTTPS media origin, migration state, and provider test assets are verified. The checked-in Blueprint keeps model/image/approval-webhook secrets on the worker, not the API; the API retains provider credentials only for authenticated diagnostics. Confirm the live dashboard separately because it was not inspected or changed here.
