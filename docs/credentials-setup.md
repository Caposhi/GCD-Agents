...
4. Back up an existing database before a migration release. The API pre-deploy runs `npm run migrate`, but the Phase 0D controller stops before API deployment whenever `LIVE_SHA..TARGET_SHA` changes `state/migrations/**`. Migration 005 is already part of the production commit; its rollout demonstrated that consumers must not race the single migration runner. For every future migration, review table volume, data changes, locks/timeouts, approval compatibility, and recovery; keep incompatible worker/scheduler processes stopped; use exactly one runner; and start compatible services only after success. Investigate a timeout or partial rollout rather than looping it.
...
During an explicitly authorized cutover, create a GitHub `production` environment and place `RENDER_API_KEY` there as a secret. Add the non-secret Render workspace/service IDs and API health URL as environment variables. Create `RENDER_DEPLOY_AUTOMATION_ENABLED=false` as a repository variable because the provenance job checks it before the deployment job enters the environment; do not put the API key in a repository variable, Render service environment, local `.env`, command argument, log, or artifact.

The live API, worker, and scheduler currently have native auto-deploy enabled. Follow the no-dual-authority procedure in [Deployment control](DEPLOYMENT.md): configure GitHub while disabled, turn native auto-deploy off on all three services, verify all three, and only then set the GitHub gate to exactly `true`. The controller blocks any new/changed migration and never runs production SQL itself.
