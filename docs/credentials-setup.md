...
Read-only verification found the GitHub `production` environment already configured with secret name `RENDER_API_KEY`, the five non-secret Render workspace/service/health variables, and the `main` deployment restriction. The secret value was not retrieved. Repository variable `RENDER_DEPLOY_AUTOMATION_ENABLED` is present and currently `false` because the provenance job checks it before the deployment job enters the environment. Do not put the API key in a repository variable, Render service environment, local `.env`, command argument, log, or artifact.

The live API, worker, and scheduler now have native auto-deploy off. The system is deliberately between authorities: GitHub is configured but not enabled or proven. Follow [Deployment control](DEPLOYMENT.md): immediately reverify all three settings off and no deployment/migration in flight, then—only under explicit authorization—set the GitHub gate to exactly `true` and perform the controlled proof. Never enable both authorities. The controller blocks any new/changed migration and never runs production SQL itself.
