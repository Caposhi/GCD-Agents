...
4. Back up an existing database before a migration release. The API pre-deploy runs `npm run migrate`, but the Phase 0D controller stops before API deployment whenever `LIVE_SHA..TARGET_SHA` changes `state/migrations/**`. Migrations `001–006` were observed applied in production on 2026-08-28, with migration `006` appearing exactly once in `_migrations` — a dated observation, the current applied set being **`UNKNOWN` unless separately verified**. `006` **must not be rerun manually** — the runner would skip it, but applying it by hand outside a transaction would also silently disable its `SET LOCAL` timeout guards. Migration 005's rollout demonstrated that consumers must not race the single migration runner. For every future migration: stop worker/scheduler, take a backup, drain or revoke old/incompatible approvals, assess actual table/media volume plus data/lock/timeout effects, run exactly one migration runner/process, start only compatible services after confirmed success, and issue fresh approvals. Investigate a timeout or partial rollout rather than looping it.
...
Read-only verification on 2026-08-24 found the GitHub `production` environment already configured with secret name `RENDER_API_KEY`, the five non-secret Render workspace/service/health variables, and the `main` deployment restriction. The secret value was not retrieved. **That row has not been reverified since** — treat the GitHub `production` environment's five non-secret variables as last-verified 2026-08-24, not current. Do not put the API key in a repository variable, Render service environment, local `.env`, command argument, log, or artifact.

The repository gate `RENDER_DEPLOY_AUTOMATION_ENABLED=false` and native auto-deploy off on the API, worker, and scheduler were **independently reverified 2026-08-28**. The system is deliberately between authorities: GitHub is configured but not enabled or proven. PR #36's worker-ownership code is **deployed — independently verified 2026-08-28** to be live in the current production release; the historical ownership-wait timing, reconciliation, and handoff behavior remain **operator-reported 2026-08-27** and were not independently re-derived. Because that deployment holds, the gate is **eligible** rather than blocked — but still not authorized here. Follow [Deployment control](DEPLOYMENT.md), which holds the authoritative ordering: reverify read-only first, then consider setting the gate to exactly `true` and performing the controlled proof, each under its own authorization. Never enable both authorities. The controller blocks any new/changed migration and never runs production SQL itself.
