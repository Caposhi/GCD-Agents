...
At that same verification the live API, worker, and scheduler had native auto-deploy off. The system is deliberately between authorities: GitHub is configured but not enabled or proven. The worker ownership fix is now reported deployed and its handoff proven (operator-reported 2026-08-27, not independently verified in an engineering session), so the gate is **eligible** rather than blocked — but still not authorized here. Follow [Deployment control](DEPLOYMENT.md), which holds the authoritative ordering: reverify read-only first, then consider setting the gate to exactly `true` and performing the controlled proof, each under its own authorization. Never enable both authorities. The controller blocks any new/changed migration and never runs production SQL itself.
