...
**The payload-contract reconciliation is implemented on a draft pull request. Do not describe it as merged, deployed, enabled, or production-validated, and do not describe the boundary it replaced as current.** One import-free authority derives every stage ceiling, exact producer/consumer guard, assembled ceiling, the **370,000** shared payload boundary, and output-token floor. The pack builder enforces 64 records; conflicts have a separate 64-entry consumer contract because 64 mutually conflicting facts yield 2,016 pair records. Over-bound packs fail closed intact before every consumer — no conflict is discarded. Text fields are bounded by UTF-8 bytes as well as characters/code units. `detail` is measured against PostgreSQL's canonical `jsonb::text` bytes, relation notes are TypeScript-validated, NULL tag elements are rejected, and migration 007's versioned helper uses collision-safe plain `CREATE`; its apply/enforce/rollback/reapply and collision paths pass on disposable PostgreSQL 16 and 18. **Neither 007 nor its rollback has run in production.** Output budgets are derived from the escaping-aware serialized byte ceilings at one token per byte — 40,000 / 79,000 / 73,000, all below the centralized 128,000-token caps — with no ordinary-prose assumption and no provider call. Stage 5's 2,200-character caption narrowing and Stage 6's ids-only `PLATFORM_CLAIMS` projection remain unchanged. **Production evidence is still only the operator's 2026-09-02 aggregate read-only audit**, which found both evidence tables empty and retrieved no raw claim text, PII, credential, or secret. The expanded `CC1`–`CC35` suite and sixteen focused mutations are repository validation, **not production validation**; all six entries remain `executionEnabled: false` and unreachable. **Next: a production-wiring design and review**, after this reconciliation is accepted. Not a new phase number.
