...
**These currently match at `44d7336…`, and may diverge again at any merge — never assume either way.** [Status](STATUS.md) is authoritative for every exact SHA and for the freshness of each mutable fact; this section states only the semantics.

- **PR #36 (worker ownership and recovery) and PR #38 (media publication normalization) are merged, deployed, and production-validated — operator-reported 2026-08-27.** The manual bootstrap was performed by the operator; no engineering session here has Render or production database access, so this is recorded as reported. The 2026-08-28 final inspection verified current service state but **did not** re-examine this bootstrap's behavioural evidence or any provider account history, so it remains **not independently verified**. The operator reported an approximately 58-second wait for exclusive ownership before readiness, reconciliation of the August 10 stranded brief with `providerMutation = impossible`, and a controlled brief in which a 896x1120 provider render normalized to 1080x1350 and reached a real human approval with nothing published automatically.
- **Phase 0B.0 is merged (`44d7336…`) and DEPLOYED.** The API, worker, and scheduler all report the target; migration 006 was applied exactly once, to the shared database, by the API pre-deploy runner at `15:24:18Z`; the worker acquired exclusive ownership and clean readiness on two separate deploys; the scheduler is live with its cron unchanged and un-triggered. The evidence tables exist and are empty, and the six reasoning stages do not execute. **Independently reverified 2026-08-28** by a separate final-inspection session with Render and read-only PostgreSQL access — see [ROLLOUT_PHASE_0B0.md §0](ROLLOUT_PHASE_0B0.md).
...
- PostgreSQL external access was open to `0.0.0.0/0` at the 2026-08-24 verification and was not revisited on 2026-08-28.
