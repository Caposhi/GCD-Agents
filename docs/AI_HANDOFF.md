...
- `state/migrations/`: forward-only PostgreSQL authority. **Migrations `001`–`006` were observed applied in production on 2026-08-28**, `006` as of that date, and **the release independently verified then carried migrations only through `006`** — see the [Phase 0B.0 rollout runbook](ROLLOUT_PHASE_0B0.md) for that release's record. That is dated evidence, not a statement about now. Separately, **`007_evidence_bounds.sql` exists in repository source through PR #54**, and **whether it is now applied to production is `UNKNOWN` — not established in either direction**, since no production database or Render service has been inspected since that date; applying it is separately authorized work. Do not read PR #54, migration 007, or the payload-contract reconciliation as deployed or production-validated.
...
1. **Phase 0B.6 — the dormant `final-critic` stage executor is `MERGED`** through PR #52. **All six target stage executors are now on `main`**, all dormant. The **payload-contract reconciliation** that gates production wiring and enablement is **`MERGED` through PR #54 and present on `main` — not established as deployed, not enabled, not production-validated**; its prerequisite is therefore satisfied in repository state only. A separately reviewed production-wiring design follows it, and that design is now **accepted and `MERGED` through PR #56** (merge `53e2c2bb6115e457670c1f99956d11a1a54530cd`) as [PRODUCTION_WIRING_DESIGN.md](PRODUCTION_WIRING_DESIGN.md) — **`UNIMPLEMENTED`**, and it authorizes nothing. **Accepted means the design document is present on `main`; it does not mean production wiring is implemented.** **No implementation PR (P1–P8) exists and no operator milestone (M1–M7) has been performed**, and each still requires its own separate authorization. No production wiring, deployment, enablement, migration application, or production validation has occurred. The design is not a new phase number, and nothing operational is implied by any merge so far. **The next step in this track is narrow:** the accepted design's **first prerequisite — a read-only verification of live production identity and database state** (the three service commits, establishing `L` and the candidate rollback artifact `R`; `/healthz`; the control settings; and **the complete migration-state evidence §4.4.2 requires** — the full `_migrations` identifier set `D` enumerated with its row count and distinct-identifier count, plus the artifact's migration files `F(A)`, so `P = F(A) − D` and all seven conditions can be evaluated; **the `_migrations` rows alone are not sufficient**, because an already-applied unexpected migration cancels out of the pending set), by an authorized operator, recorded as a dated observation. It is separately authorized and **has not been granted**. The migration-007 audit and decision record, confirmation of same-commit `preDeployCommand` behaviour, and rollback-artifact compatibility evidence follow it, each separately. **M1 is not authorized.** **The read-only readiness *evidence package* for that prerequisite is now `MERGED` through PR #60** (merge `2a9edb7f86a07ddb7c27bc91e3c214052d0a2dc4`) as [M1_READINESS_DECISION_RECORD.md](M1_READINESS_DECISION_RECORD.md) plus two checked-in read-only operator scripts. Its verdict is **`M1 BLOCKED / NO-GO`**, and merging it **completed the package, not the readiness**: it established none of `A`, `L`, the service identities, the production migration state, the aggregate audit, or rollback compatibility, and it authorized and executed nothing. Migration 007's applied state remains **`UNKNOWN` in either direction**. The post-merge deployment workflow refused at the disabled-automation gate exactly as designed — the controller never ran and no Render deployment occurred through it. See [Status](STATUS.md) for the recorded workflow evidence.
