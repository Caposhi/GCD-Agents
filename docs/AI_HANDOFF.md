...
- `src/harness/evidence/`: the Phase 0B.0 evidence contract, pack builder, approved-facts adapter, and the `evidence:sync` operator command. `src/harness/agents/registry.ts`: the six-stage agent registry, asset resolution, and stage planning — **no stage executes**. `src/harness/contentIntelligence.ts`: the deterministic, inert preview those two feed. Merged and live on the API.
- `src/mcp/`: imported provider libraries, not standalone MCP servers/model tools.
- `state/migrations/`: forward-only PostgreSQL authority. **001–006 are applied in production**, 006 as of 2026-08-28. The release carrying it is only partially rolled out — see [Phase 0B.0 rollout runbook](ROLLOUT_PHASE_0B0.md).
...
- **Phase 0B.0 is merged (`44d7336…`) and PARTIALLY deployed.** Operator-verified 2026-08-28: the API is live at the target and migration 006 was applied exactly once at `15:24:18Z`; the **worker and scheduler remain at `a6a4316…`**. That mixed version is a proven-compatible safe pause state. The evidence tables exist and are empty, and the six reasoning stages do not execute. Not independently verified in an engineering session — see [ROLLOUT_PHASE_0B0.md §0](ROLLOUT_PHASE_0B0.md).
...
**Phase 0B.0 — content evidence and agent foundation (PR #40, merge `44d7336…`).** `MERGED`, **partially `DEPLOYED`** — API only. Migration 006, the typed evidence contract with its eight epistemic kinds, the evidence pack that surfaces conflicts instead of resolving them, the deterministic approved-facts projection, the explicit `evidence:sync` operator command, the six-stage agent registry with allowlist-rooted asset loading, and an inert Content Intelligence preview route. **No stage executes and no model call was added.**

## 6. Current operation in progress

**The first migration-bearing production rollout — paused mid-flight.** Phase 0B.0 merged as `44d7336…` on 2026-08-27 and was partially rolled out on 2026-08-28: API at the target, migration 006 applied once at `15:24:18Z` in ~53 ms, evidence tables empty, zero active briefs and approvals. It **stopped at step 6** under S8/S18 because the runbook claimed 9 indexes where the catalog reports 10 — the schema was correct, the document was wrong, and the stop is the safety mechanism working. **Worker and scheduler remain at `a6a4316…`.** Resuming needs independent inspection of the corrective delta, a fresh read-only preflight, fresh authorization, and restart at the worker deployment; migration 006 must not be rerun. See [ROLLOUT_PHASE_0B0.md §0](ROLLOUT_PHASE_0B0.md).
...
1. Have the corrective documentation delta on PR #41 independently inspected, then read-only reverify the production half of the preflight — Render service states, native auto-deploy off, all three live SHAs, `/healthz` identity, `_migrations` = `001–006`, brief/approval queue state, worker ownership, and recent errors. That half cannot be verified from an engineering session (no Render access; `/healthz` and `api.render.com` egress denied) and is the gate on everything else. Stop on any discrepancy.

Only after that, and only under fresh explicit authorization, **resume [ROLLOUT_PHASE_0B0.md](ROLLOUT_PHASE_0B0.md) at step 8, the worker deployment**, against exactly `44d7336f2c75ff880cff0d8205d2fafe13eb91b5`. Steps 4–7 are complete; **migration 006 is applied and must not be rerun**.
