...
- **Phase 0B.6 — `IMPLEMENTED` in source on draft PR #52, `NOT MERGED`.** `src/harness/agents/finalCritic.ts` with `agents/final-critic.md` and `skills/critique-discipline/SKILL.md`: the sixth and last stage on the same boundary. Unlike the five stages above, this one is **not yet on `main`** — it exists only on a draft, unmerged pull request pending independent review. All six target stage executors now exist in source; only the first five are merged, and all six remain dormant.
...
**These currently differ, and never assume either way.** Production was last independently verified at `44d7336…` on 2026-08-28; the source lineage now additionally carries the merged, dormant Phase 0B.1 through Phase 0B.5 executors, none established as deployed, plus a draft, unmerged Phase 0B.6 `final-critic` executor (PR #52) implemented but not on `main`. [Status](STATUS.md) is authoritative for the freshness of mutable production facts; the exact current `main` is a Git lookup, not a documented field. This section states only the semantics.
...
1. **Phase 0B.6 — the dormant `final-critic` stage executor is now `IMPLEMENTED`** in source, on draft, unmerged PR #52. All six target stage executors now exist in source; only the first five are merged. The next steps are a **separately scoped payload-contract reconciliation**, which is a hard prerequisite to enabling any stage, and then a **separately scoped production-wiring design/review** — neither started, neither named as a new phase number, and neither implied by this implementation.
...
2. **merged to `main`** — true for the first five executors only; `final-critic` sits on draft, unmerged PR #52;
...
### Phase 0B.6 — implemented in source, draft, `NOT MERGED`

The `final-critic` stage executor is **`IMPLEMENTED`** in source on draft PR #52. Unlike Phase 0B.1–0B.5, it is **not on `main`** — merge, enablement, deployment, and production validation are all separately scoped future work. When (and if) it merges, it will be dormant on the same terms as the other five: nothing reaches it — worker, scheduler, orchestrator, API, preview, approval, publication, provider, media, Slack, database, and evidence-write paths are all asserted unable to — and `executionEnabled` is unchanged for every stage.
...
**Do not repeat only half of the payload-boundary fact.** Stage 6's own `packagingOutputChars` ceiling is now safely above every Stage 5-valid serialized package, **and** Stage 6 as a whole still does not accept every structurally valid Stage 5 handoff: the *shared* assembled-payload boundary `MAX_PAYLOAD_CHARS` (120,000) in `stageExecution.ts` applies to the whole framed payload and refuses a maximal Stage 5 output before any model call, with zero model requests. Never state either half alone. There is also **no fixed packaging envelope to quote**: `SCRIPT_CLAIMS` and `PLATFORM_CLAIMS` have no finite structural maximum while `EvidenceRecord.claim` is unbounded (non-empty in TypeScript, `length(btrim(claim)) > 0` in migration 006) and a claim bound for several platforms is projected once per platform, so the available packaging payload is `MAX_PAYLOAD_CHARS` minus the serialized sizes of the other five framed blocks — zero for large enough valid evidence text. Any number quoted for it is an example for particular claim lengths, not a contract. The same producer/consumer guard mismatch already exists between the **merged** stages (Stage 1 → Stages 2/3, Stage 2 → Stage 3, Stage 3 → Stages 4/5/6, Stage 4 → Stages 5/6), and an equivalent mismatch exists on the output side between the stage output contracts and their configured 2,000- and 3,000-token model budgets. **These are accepted dormant limitations, not production validation.** A separately scoped **payload-contract reconciliation** — bounding evidence text in TypeScript and PostgreSQL after a read-only data audit, with explicit migration and rollback; deriving every producer/consumer handoff guard from the producer contract and deriving the shared assembled boundary; reconciling output contracts with model `maxTokens`; adding derivation regressions against future drift; and considering narrower Stage 6 projections where they preserve the critique contract — is a **hard prerequisite to production wiring and to enabling any stage**. It is not a new phase number and is not started.

## 8. Roadmap

[Roadmap](ROADMAP.md) is authoritative and binding — see the roadmap-continuity rule in [`AGENTS.md`](../AGENTS.md). The Phase 0B evidence contract is implemented (Phase 0B.0). All six target stage executors now exist in source; five are merged and dormant, and the sixth — `final-critic` — is implemented on a draft, unmerged PR (#52) and is not yet on `main`. Remaining work also includes provider idempotency/operation ledger; provider reconciliation; PostgreSQL network restriction; token lifecycle; control/reviewer identity; retention and backup/restore; the external readiness register; and the deployment-authority cutover proof. The worker lease/reaper item is superseded and is no longer active work.

## 9. Outstanding risks

Highest priority: no durable provider idempotency, operation ledger, or reconciliation — provider-level `withRetry` can still reissue a request after an ambiguous network outcome, so duplicate publication remains possible. Worker-interruption recovery is deployed — its code is live in the current `44d7336…` release, independently verified 2026-08-28 — so that specific stranding mode should be closed; the earlier bootstrap's behavioural evidence (the ownership-wait timing, the August 10 reconciliation) remains operator-reported 2026-08-27 and was not independently re-examined, and should be reconfirmed before being relied on. PostgreSQL external access remains `0.0.0.0/0`, independently reverified 2026-08-28; the default-path Instagram token persists in plaintext; approval uses a bearer URL with a generic reviewer identity; one shared control secret carries process-local rate limits; there is no retention or restore drill; and provider ownership, scopes, versions, and backup facts remain unverified. Skills are not injected into any **production** model call — all six implemented Phase 0B executors inject their reviewed prompt and skill assets, but all remain dormant and unreachable, and only five are merged to `main`. Scorecard/proposal tables are unwritten and no empirical learning runtime exists. Phase 0B.0 adds the evidence substrate but does not populate it: migration 006 is applied, `content_evidence` is empty until an authorized operator runs `evidence:sync` (not yet run), and **no reasoning stage executes in production**. The rollout is complete; production is no longer at a mixed version.

## 10. Content Intelligence target architecture

Use about six primary reasoning stages—strategy-concept, automotive-truth, hook-story-script, production-direction, packaging-adaptation, final-critic—surrounded by deterministic retrieval, evidence, policy, validation, state, and publication services. The roughly 22 researched roles are conceptual capabilities, not 22 mandatory calls. Phase 0B.0 added the `AgentRegistry`, its checked-in asset resolution for all six stages, the durable evidence contract, and a deterministic inert preview. **Phase 0B.1 through Phase 0B.5 add five merged executors on the shared boundary, and Phase 0B.6 adds the sixth and last — `final-critic` — implemented in source on a draft, unmerged PR (#52).** All six inject their reviewed prompt and skill assets into the instruction channel while factual reference assets remain outside it. Retrieval and evidence capture from real runs remain unbuilt. **No stage executes in any production path**, so this is still target architecture, not current runtime. The next step is a separately scoped production-wiring design/review, not started.
