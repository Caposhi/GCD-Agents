...
- `state/migrations/`: forward-only PostgreSQL authority. **Migrations `001`–`006` are applied in production**, `006` as of 2026-08-28, and **the independently verified production release carries migrations only through `006`** — see the [Phase 0B.0 rollout runbook](ROLLOUT_PHASE_0B0.md) for that release's record. Separately, **`007_evidence_bounds.sql` exists in repository source through PR #54 and is not applied to production**: no deployed release carries it, and applying it is separately authorized work. Do not read PR #54, migration 007, or the payload-contract reconciliation as deployed or production-validated.
