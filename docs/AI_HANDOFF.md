...
**The payload-contract reconciliation is implemented on a draft pull request. Do not describe it as merged, deployed, enabled, or production-validated, and do not describe the boundary it replaced as current.** What it establishes: every bound the six stages apply comes from one module, `src/harness/agents/payloadContract.ts`, which imports nothing; each stage's serialized ceiling is derived from a shape witness plus its string-content allowance times `MAX_JSON_ESCAPE_EXPANSION` (**2**, provable because every bounded string field now refuses control characters and unpaired surrogates); every consumer's handoff guard is **exactly** its producer's ceiling, asserted by equality rather than sufficiency; `MAX_PAYLOAD_CHARS` is the largest assembled stage ceiling rounded to the next 10,000 — **370,000**, replacing the hand-chosen 120,000; evidence text is bounded in TypeScript and mirrored by additive migration `007_evidence_bounds.sql`, with `state/rollback/007_evidence_bounds_rollback.sql` as the documented reversal (**neither applied to any database**); the three policy token budgets are derived from the output contracts (8,000 / 15,000 / 15,000, replacing 4,000 / 3,000 / 2,000) at a conservative three characters per token; Stage 5 captions are narrowed to a 2,200-character pipeline cap enforced as `min(provider, pipeline)`; and Stage 6's `PLATFORM_CLAIMS` carries fact **ids** only, with the final-critic authority contract — platform membership and order, the duplicate-triple guard, fact binding, owner consistency, the five literal-`false` brands, the zero-used-claims refusal — unchanged. **Production evidence: a read-only, aggregate-only audit only**, run independently by the operator on 2026-09-02 against the Render database; both evidence tables were empty, no credentials were requested or received, and no raw claim text or PII was retrieved. That audit is why migration 007 may validate immediately; it justifies **no particular number**. The bounds are derived and regression-tested (`CC1`–`CC28`, plus `npm run test:payload-mutation`), **not production-validated**: every stage still reports `executionEnabled: false` and nothing reaches an executor. **Next: a production-wiring design and review**, after this reconciliation is accepted. Not a new phase number.
