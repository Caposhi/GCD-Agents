...
The separate GitHub `production` environment was verified to hold secret name `RENDER_API_KEY` and non-secret variables `RENDER_WORKSPACE_ID`, `RENDER_API_SERVICE_ID`, `RENDER_WORKER_SERVICE_ID`, `RENDER_SCHEDULER_SERVICE_ID`, and `RENDER_API_HEALTH_URL`, with deployment restricted to `main`. The secret value was not retrieved. The health variable is not a free-form destination: the controller accepts only `https://gcd-social-api.onrender.com/healthz`. Repository variable `RENDER_DEPLOY_AUTOMATION_ENABLED` is deliberately outside the environment because the provenance job checks it before the deployment job enters that environment. The gate was `false` and all three Render native settings were off at the 2026-08-24 read-only verification, leaving the intended zero-authority window, and both were **independently reverified 2026-08-28**: the gate remains `false` and native auto-deploy remains off on all three services. Only a separately authorized change to exact string `true` permits the controller to continue. The manual ownership bootstrap that gated this is reported complete, so the change is now **eligible** — but it remains **separately unauthorized** and requires a fresh read-only preflight immediately before acting. Exact values, the bootstrap procedure, and the remaining proof steps are in [Deployment control](DEPLOYMENT.md).
