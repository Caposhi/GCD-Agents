...
| Variable | Behavior | Safety |
|---|---|---|
| `ANTHROPIC_API_KEY` | Enables model and vision calls | Cost and content egress. The dormant Content Intelligence stages resolve their models in `src/harness/agents/modelPolicy.ts`, not from any variable: `reasoning-heavy` → `claude-opus-5`, `reasoning-standard` → `claude-sonnet-5` (both thinking disabled), `critic` → `claude-opus-5-5` (adaptive thinking, effort `high`, `max_tokens` 128,000; with the narrow critic panel, `IMPLEMENTED` and not merged, `final-critic` makes four such requests — one per lens — so a critic run costs up to four times one request). No deployed service resolves those policies — they run only from the operator-local `scripts/local/content-run.mjs`, whose live mode also requires a typed `LIVE` confirmation — so none of this changes what a deployed service sends |
| `IMAGEGEN_API_KEY` | fal.ai image generation | Required for a publishable image; cost and prompt/image egress |
| `MANAGER_MODEL` | Dormant `agentLoop.ts` manager model override; default is `claude-opus-5` | Not used by production worker. **Recorded consequence, not yet handled:** `runManagerTurn` never sets `opts.thinking`, and `buildRequest()` in `sdk.ts` resolves `thinking` through `resolveLegacyThinking()`, which pins `{ type: "disabled" }` only for models listed in its `THINKING_ON_OMISSION` set and otherwise sends no `thinking` key at all. **`claude-opus-5` is deliberately not in that set**, so this dormant path is unaffected and the consequence below still stands. On the prior default `claude-opus-4-8`, omitting `thinking` ran no thinking; on the current default `claude-opus-5`, omitting it runs adaptive thinking by default. This legacy path is non-streaming with `max_tokens` defaulting to 3000 and a 90-second timeout, so a reviver of this dormant code must set `opts.thinking` explicitly (or otherwise account for adaptive thinking's added token/latency cost) before calling `runManagerTurn`, or risk truncated output or a timeout |
...
The separate GitHub `production` environment was verified to hold secret name `RENDER_API_KEY` and non-secret variables `RENDER_WORKSPACE_ID`, `RENDER_API_SERVICE_ID`, `RENDER_WORKER_SERVICE_ID`, `RENDER_SCHEDULER_SERVICE_ID`, and `RENDER_API_HEALTH_URL`, with deployment restricted to `main`. The secret value was not retrieved. The health variable is not a free-form destination: the controller accepts only `https://gcd-social-api.onrender.com/healthz`. Repository variable `RENDER_DEPLOY_AUTOMATION_ENABLED` is deliberately outside the environment because the provenance job checks it before the deployment job enters that environment. The gate was `false` and all three Render native settings were off at the 2026-08-24 read-only verification, leaving the intended zero-authority window, and both were **independently reverified 2026-08-28**: the gate remained `false` and native auto-deploy remained off on all three services. **Native auto-deploy off on all three services was independently reverified again 2026-09-18**, as part of the M1 verification (see [Status](STATUS.md)); the gate variable itself was read in the authenticated GitHub UI on 2026-09-18 as exact lowercase `false`, and its last-updated timestamp predates M1 (2026-09-17), so the variable was not changed by or during M1. Separately, that M1 deploy was itself confirmed `trigger: "manual"` (Render dashboard), not driven by this GitHub controller. Only a separately authorized change to exact string `true` permits the controller to continue. The manual ownership bootstrap that gated this is reported complete, so the change is now **eligible** — but it remains **separately unauthorized** and requires a fresh read-only preflight immediately before acting. Exact values, the bootstrap procedure, and the remaining proof steps are in [Deployment control](DEPLOYMENT.md).

The separate GitHub `monitoring` environment holds one secret, `GCD_MONITOR_DATABASE_URL`, used only by `.github/workflows/interval-monitor.yml`. It is a read-only PostgreSQL role with `SELECT` on `_migrations` and on four columns of `brief_queue`, and nothing else; it is **not** the application's `DATABASE_URL` and must never be copied into a Render service environment, `.env`, documentation, or a fixture. It is an **environment** secret rather than a repository secret, and that environment is restricted to `main`, so the monitor cannot be exercised end to end from a branch — the job declares `environment: monitoring`, without which the secret is unreachable. That restriction must not be relaxed, and the secret must not be moved to the repository level, to make branch testing possible. The monitoring workflow also reads the existing repository variable `RENDER_DEPLOY_AUTOMATION_ENABLED`, comparing it as an exact string rather than setting it. Its permissions are `contents: read` and `actions: read`; it has no `pull_request` trigger, so a fork's pull request can never reach the secret. See [Operations](OPERATIONS.md).
