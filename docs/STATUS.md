...
**The payload-contract reconciliation is implemented on a draft pull request — not merged, not deployed, not enabled, and not production-validated.** Not a new phase number. It bounds evidence text in TypeScript and in an additive migration 007 with an explicit rollback file (**neither applied to any database**); derives every producer/consumer handoff guard and every assembled-payload ceiling from a single authority, `src/harness/agents/payloadContract.ts`, so `MAX_PAYLOAD_CHARS` is now the derived **370,000** rather than a hand-chosen 120,000; reconciles the six output contracts with the model token budgets, which become the derived 8,000 / 15,000 / 15,000 rather than 4,000 / 3,000 / 2,000; narrows Stage 6's `PLATFORM_CLAIMS` projection to fact ids while leaving the final-critic authority contract intact; narrows Stage 5 captions to a 2,200-character pipeline cap rather than Facebook's 63,206; and adds `CC1`–`CC28` derivation regressions plus a nine-mutation harness (`npm run test:payload-mutation`) that proves them load-bearing. **Production evidence: a read-only, aggregate-only audit only, run independently by the operator on 2026-09-02**, which found `content_evidence` and `content_evidence_relations` both empty; no credentials were requested or received and no raw claim text or PII was retrieved. **Next: a production-wiring design and review**, after that reconciliation is accepted. Enablement, deployment, and production validation remain future, separately authorized work, and none may proceed before both. Deployment-authority work remains an independent track and must not be combined with either.
