...
- The separate production workflow is serialized and initially disabled. It admits only the exact successful same-repository `CI` result for a `main` push, rejects stale results after the concurrency wait before any Render command, and once explicitly cut over derives `LIVE_SHA`, handles already-current production without redeploy, blocks migration changes, and performs exact-SHA API → exact-origin/target-identity health → target-bound worker readiness/10-second stabilization → scheduler → final-SHA sequencing. Its bounded diagnostics use recursive JSON plus fallback secret redaction and a single Markdown-inert renderer.
