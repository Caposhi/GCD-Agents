...
**Since that dated baseline, one semantic fact changed that the table does not carry:** PR #54 merged `007_evidence_bounds.sql` to `main`. The file **exists in source and is not applied to production**; production still runs migrations `001–006`. That is a repository fact created by a merge, not a refresh of the baseline SHA above, which is deliberately left as the dated evidence it is.
...
| **Payload-contract reconciliation** — PR #54, merge `0c13ab1af9c7ca796a1d48ed37207715a47166e4` | **`MERGED`** — **not `ENABLED`, not established as `DEPLOYED`, not `PRODUCTION-VALIDATED`**; the prerequisite to production wiring is satisfied in **repository state only**. Migration 007 exists in source and is **not applied to production**; production evidence: none |
...
**Phase 0B.6 is `MERGED` through PR #52 (merge `ec2a0b78e84386d5491db96418a53c64ee3a8e08`), deliberately dormant, not enabled, and not established as deployed or production-validated.** It is present on `main` on the same terms as Phase 0B.1–0B.5; merge is repository evidence only, and **production evidence is none**. With it, **all six target stage executors are merged and every registry entry reports `executionEnabled: false`.** Nothing reaches the `final-critic` executor through the worker, scheduler, orchestrator, API, preview, approval, publication, provider, image, Slack, database, or evidence-write paths. It changed no `executionEnabled` field and added no route, migration, environment variable, credential, dependency, workflow change, `render.yaml` change, provider payload, approval logic, or scheduling behaviour. **Stage 3's actually used claims remain the complete factual authority, narrowed further by stage 5's own per-platform bindings (`PLATFORM_CLAIMS`)** — the entire prior chain (stages 2–5) is revalidated before any model call, and the requested-platform sequence is separately checked against stage 5's own package sequence. Its critique carries five literal-`false` fields (`authoritative`, `approvalGranted`, `publishable`, `executable`, `productionValidated`) that the output contract gives a model no field to escape; it does not replace the existing `brand-compliance-critic` publishing gate, which is unchanged. It still does not prove a finding correct, that the package actually has the described problem, or that the verdict is the right call. **Payload boundary at that merge — since addressed and now merged through PR #54:** as merged, Stage 6 did not accept every structurally valid Stage 5 handoff, because the shared assembled-payload boundary `MAX_PAYLOAD_CHARS` (120,000) refused a maximal Stage 5 output before any model call, and no fixed packaging envelope existed to quote while `EvidenceRecord.claim` was unbounded. The same producer/consumer guard mismatch existed between merged Stages 1–5, and an equivalent one between the stage output contracts and their configured model token budgets. **The payload-contract reconciliation that closes all of that is `MERGED` through PR #54** — see the next paragraph. See [Roadmap](ROADMAP.md).

**The payload-contract reconciliation is `MERGED` through PR #54 (merge `0c13ab1af9c7ca796a1d48ed37207715a47166e4`) and present on `main` — not established as deployed, not enabled, and not production-validated.** Merge changed repository state only; it authorizes no production wiring, no enablement, and no migration application. Its single authority derives the **370,000** assembled boundary and exact handoff guards. It enforces 64 projected records and, separately, 64 conflict entries; exhaustive detection still emits all 2,016 pairs for 64 mutually conflicting facts, then every consumer refuses that intact over-bound pack before a model call. Evidence records are revalidated at pack construction, durable-row reconstruction, pack projection, and the shared pre-model boundary. Text limits also cap UTF-8 bytes. The 4,000-byte `detail` contract is PostgreSQL-canonical `jsonb::text`, including the 327-byte signed `-5e-324` worst case; relation notes are validated in TypeScript, NULL tag elements are rejected, and migration 007's uniquely versioned helper is collision-safe. Disposable PostgreSQL 16.15 and 18.6 each pass 208 apply/enforce/rollback/reapply and collision checks (fresh 59, upgrade 80, durable 69); **neither 007 nor its rollback has run in production**. The six output budgets are derived from escaping-aware serialized byte ceilings at one token per byte: 40,000 / 79,000 / 73,000, below the centralized 128,000-token caps. The typed centralized stage policy explicitly disables Anthropic thinking because adaptive thinking shares `max_tokens`; the exact production request is tested, while legacy `runAgent` and vision behavior remain unchanged. **A stage call is now one provider request, not one wrapper invocation**: the SDK defaults `maxRetries` to 2, so the stage boundary sets it to 0 explicitly and streams, because the declared `max_tokens` cannot be received on a non-streaming connection. Two bounds are named apart because the SDK implements only one: its `timeout` covers request setup and is cleared when streaming headers arrive, so the stage arms its own budget-derived total deadline (35 / 67 / 62 minutes), aborts the stream when it expires, and clears the timer on both the success and failure paths. Twenty output tokens per second is an explicit operational assumption, not a measured rate. **A pack must now mean what its shape claims**: `assertUsableEvidencePack` is the one runtime validator every consumer calls, enforcing section-permitted kinds, lifecycle in both directions, one section per record, `allowedFacts` restricted to active fresh verified facts, conflicted material held in a dedicated non-citable `conflictedEvidence` section so a conflict's ids always resolve, stale/inactive material barred from remaining usable, every conflict field bounded in code units and UTF-8 bytes with its `aClaim`/`bClaim`/`subject` checked as exact snapshots of the records it names, the closed `basis` enum, honest `counts`, the pack's `goal`, and a round-tripping ISO-8601 `builtAt` — the documented anchor for freshness. Before it, a valid hypothesis hand-placed in `allowedFacts` reached a stage runner and was cited as established fact. Stage 5's 2,200-character caption narrowing and Stage 6's ids-only projection remain. `CC1`–`CC81` and thirty-three mutations across nine files prove the corrections load-bearing. **Production evidence: none** for the reconciliation itself; the only production evidence touching it remains the operator's dated 2026-09-02 aggregate read-only audit, which found both evidence tables empty and retrieved no raw claims, PII, credentials, or secrets — a dated fact, not a statement that production now runs this merge. **Migration 007 exists in source and has not been applied to production**; applying it needs its own authorization, a fresh production audit, the separately authorized migration-bearing rollout, and post-application verification. **Next: a separately reviewed production-wiring design** — a first draft now exists as [PRODUCTION_WIRING_DESIGN.md](PRODUCTION_WIRING_DESIGN.md), which is a design only and authorizes nothing. Performance ingestion, governed learning, and the proposed future Google Business Profile expansion remain later work and are not begun by this reconciliation. All six stages remain unreachable and `executionEnabled: false`, with no production path reaching them; deployment-authority work remains separate.
