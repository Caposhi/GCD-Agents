...
**The payload-contract reconciliation is implemented on draft PR #54 — not merged, not deployed, not enabled, and not production-validated.** Its single authority derives the **370,000** assembled boundary and exact handoff guards. It enforces 64 projected records and, separately, 64 conflict entries; exhaustive detection still emits all 2,016 pairs for 64 mutually conflicting facts, then every consumer refuses that intact over-bound pack before a model call. Text limits also cap UTF-8 bytes. The 4,000-byte `detail` contract is PostgreSQL-canonical `jsonb::text`, relation notes are validated in TypeScript, NULL tag elements are rejected, and migration 007's uniquely versioned helper is collision-safe. Disposable PostgreSQL 16 and 18 each pass 196 apply/enforce/rollback/reapply and collision checks; **neither 007 nor its rollback has run in production**. The six output budgets are derived from escaping-aware serialized byte ceilings at one token per byte: 40,000 / 79,000 / 73,000, below the centralized 128,000-token caps. Stage 5's 2,200-character caption narrowing and Stage 6's ids-only projection remain. `CC1`–`CC35` and sixteen mutations prove the corrections load-bearing. **Production evidence remains only the operator's 2026-09-02 aggregate read-only audit**, which found both evidence tables empty and retrieved no raw claims, PII, credentials, or secrets. **Next: a production-wiring design and review**, only after this reconciliation is accepted. All six stages remain unreachable and `executionEnabled: false`; deployment-authority work remains separate.
