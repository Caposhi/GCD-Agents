...
**The M1→M2 partial-release interval is now explicitly bounded, owned, and monitored — recorded 2026-09-18 in [Status](STATUS.md#m1m2-interval--owner-bound-and-monitoring-recorded-2026-09-18) and [Roadmap](ROADMAP.md#m1m2-interval--owner-bound-and-monitoring-recorded-2026-09-18).** In brief: owner Michael Capote (CTO, Alan Gelfand Inc. DBA German Car Depot), interval start `2026-09-17T18:52:47.893627Z`, expiry `2026-09-24T18:52Z` as a decision point requiring its own explicit authorization (not a self-executing trigger), daily read-only monitoring of deploy activity/migration set/API health/scheduler cron, and the recovery path of redeploying exact `R` = `44d7336f2c75ff880cff0d8205d2fafe13eb91b5` to the API only (which does not unapply migration 007). This record does not authorize M2, re-authorization, or the recovery path — see the linked sections for the full text and required conditions.

**`main`/production divergence.** `main` is now `af513b5bbe89b7a60c7fb4929db325a566db7d41` (post-PR-#68), documentation-only relative to the prior head; the deployed API artifact remains exact `A` = `d5015236672a02bf8f58d342625c32a4f5acc8a1`. These are no longer the same commit, so "deploy `main`" and "deploy `A`" are different instructions — name which one is meant. `main` is not deployed.
