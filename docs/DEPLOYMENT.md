...
Phase 0D is merged and production-deployed; it does not begin Phase 0B. **API current exact target: `A` = `d5015236672a02bf8f58d342625c32a4f5acc8a1`**, reached via M1 (an API-only deployment, independently verified 2026-09-18) — deploy `dep-dam3dfv40ujc73fgidhg`, `trigger: "manual"`; migration `007_evidence_bounds.sql` applied inside that window. **Worker and scheduler current exact target remains `R` = `44d7336f2c75ff880cff0d8205d2fafe13eb91b5`**, reached through the Phase 0B.0 migration-bearing rollout and deliberately unchanged by M1. Independently verified 2026-08-28 by a separate final-inspection session with Render and read-only PostgreSQL access, and reverified 2026-09-18 for the M1 facts above: workspace `tea-d4fkclpr0fns73abmnh0`, API `srv-d8u0qtpo3t8c73c5o44g` at `A`, worker `srv-d8u0qtpo3t8c73c5o440` and scheduler `crn-d8ulb4rtqb8s73bdjctg` at `R`, PostgreSQL `dpg-d8u0qaho3t8c73c5nj40-a` holds `_migrations` `001–007`. See [Status](STATUS.md) for the full M1 record, including its Tier 3 open limitations. The GitHub `production` environment's five non-secret variables are the only row still carrying only the earlier 2026-08-24 21:32 UTC verification and were not revisited — reconfirm that one read-only immediately before any production operation. **M1 did not authorize and did not begin M2.**

**The M1→M2 partial-release interval is explicitly bounded and owned, and has been monitored as scoped since its 2026-09-22 re-authorization — recorded 2026-09-18 and re-authorized 2026-09-22 in [Status](STATUS.md#m1m2-interval--owner-bound-and-monitoring-recorded-2026-09-18) and [Roadmap](ROADMAP.md#m1m2-interval--owner-bound-and-monitoring-recorded-2026-09-18).** In brief: owner Michael Capote (CTO, Alan Gelfand Inc. DBA German Car Depot), interval start `2026-09-17T18:52:47.893627Z`; the original expiry `2026-09-24T18:52Z` is **superseded** by the owner's explicit re-authorization under outcome (b), with the new bound `2026-10-22T18:52Z` — still a decision point requiring its own explicit authorization (not a self-executing trigger). Monitoring for the prior interval went unmet; for the extended interval the daily read-only `Interval monitor` workflow (`.github/workflows/interval-monitor.yml`, scheduled `0 14 * * *`) covers the API's artifact and health, the migration set, scheduler liveness, the deployment gate and `deploy-production` refusals, but not worker/scheduler deploy identity or the Render native auto-deploy setting, which remain covered by human verification passes, and its scheduled trigger has not yet been observed firing — see Status and Roadmap for the full scope, the compensating controls, and the verification passes scheduled before the new bound; and the recovery path of redeploying exact `R` = `44d7336f2c75ff880cff0d8205d2fafe13eb91b5` to the API only (which does not unapply migration 007). This record does not authorize M2, re-authorization, or the recovery path — see the linked sections for the full text and required conditions.

**`main`/production divergence.** The deployed API artifact is exact `A` = `d5015236672a02bf8f58d342625c32a4f5acc8a1`. `main` is ahead of `A` by documentation-only commits — the exact current `main` is a Git/GitHub lookup, not a field this file maintains: run `git rev-parse origin/main`. "Deploy `main`" and "deploy `A`" are different instructions — name which one is meant. `main` is not deployed.
...
| API | **`A` = `d5015236…`**, live and healthy — redeployed via M1 | independently verified 2026-09-18 |
| Worker | **`R` = `44d7336…`** — exclusive ownership held and readiness reported on two separate deploys; unchanged by M1 | independently verified 2026-08-28 that ownership is held and readiness is reported at this target (this authoring session has no Render access; a separate final-inspection session did); the exact acquisition timings (58,142 ms, 60,094 ms) and the recovery/readiness event sequence are operator-reported, not independently re-derived; live at `R` reconfirmed 2026-09-18 |
...
1. **Merge to `main` with exact-head CI green. COMPLETE** — PR #36, reviewed head `281eb8f232995e58e404c916c3ec0a23b62c7acc`, all five CI jobs green, merged as `0828cc91c41c9cd10ad709db30491ada0a52c811`.
2. **Read-only production reverification.** Reconfirm current `main`, all three live SHAs, native auto-deploy off on all three services, the GitHub gate still `false`, the `production` environment configuration, and no deployment or migration in flight. Stop on any discrepancy.
3. **Reconcile the August 10 incident against provider account history first.** Check Instagram, Facebook, and Google Business Profile directly for the 2026-08-11 Mini Cooper check-engine content before mutating any production row. Public search was inconclusive and is not sufficient evidence. Establishing whether anything was published is a prerequisite for deciding how the row is resolved, not a formality after it.
4. **Then, under its own explicit production authorization, resolve the stale `running` row** (`c5e53afe-2657-4e11-811d-53ce5e793245`), so no unexplained `running` row remains for the reconciler's first firing to act on as a deployment side effect. A production write coupled to a deployment is exactly the ambiguity this sequence exists to avoid.
5. **Read-only preflight:** zero `running` briefs; **zero pending approvals**; no deployment in progress; far from the 13:00 UTC scheduler window; no manual trigger expected.
