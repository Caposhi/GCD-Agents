...
5. **Two prompts that described the lists before they bound the writers now say what is true** (an owner-directed addendum on the same branch; both were first recorded here as accepted limitations and are fixed in this change):
   ...
   - `agents/automotive-truth.md` no longer says "`forbiddenClaims` is advisory: it tells later stages and human reviewers what you rejected and why." It says `requiredCaveats` and `forbiddenClaims` are passed to stages 3–5 as binding restrictions and to the critic as its yardstick, so each should be written precisely and only where the evidence warrants it. The next two sentences are kept verbatim: "It is not a filter anything runs, so a claim you leave out of it is not thereby permitted. Nothing is permitted except what you bound to a fact id." (The kept "it" now follows two lists; it was kept verbatim as directed.)
...
- ~~An owner decision on the evidence-lens prompt sentence and the stage 2 prompt's "advisory" wording.~~ **Fixed in this change** by the owner-directed addendum (Delivered, item 5; `CM11`).
