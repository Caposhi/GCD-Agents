...
**State:** **`IMPLEMENTED`** — proposed in a **draft** pull request. **Not `MERGED`, not `ENABLED`, not `DEPLOYED`, not `PRODUCTION-VALIDATED`.** It is not on `main`. **Production evidence: none.**
...
**Deterministic policy is reused, not redeclared.** The per-platform caption and hashtag limits are **imported from `packageMap.ts`**, the module that already enforces them on provider text — Instagram 8–15 hashtags and 2,200 characters, Facebook at most 2, Google Business Profile zero hashtags and 1,500 characters. `packageMap.ts` now exports those constants and names its two previously inline literals; behaviour there is unchanged, verified by the existing suites. **No competing value is defined anywhere.**

**A naming divergence, recorded rather than hidden.** The repository's provider-payload `Platform` union spells Google Business Profile **`gbp`**. This stage's closed enum spells it **`google_business_profile`**, because a stage-5 package is review metadata and must never be mistaken for a publishable payload. `PACKAGING_PLATFORM_PRODUCTION_ID` pins the one-to-one correspondence and a test asserts it is total and injective, so adding, removing or renaming a supported platform fails loudly instead of leaving two silently divergent vocabularies.
...
**Testing.** The offline suite grew from 560 to **731** checks in `test:content-intelligence` (**1,016** across all eight offline suites), every model call through an **injected runner**. Confirmed to pass with a **nonempty `ANTHROPIC_API_KEY`** *and* `ANTHROPIC_BASE_URL` pointed at an unreachable local port. Five deliberate mutations of the executor were used to confirm the safety assertions are load-bearing — removing the zero-claims refusal, the platform-order check, the aggregate handoff bound, the hashtag policy, or the used-claim membership check each fails specific named checks, and the executor was restored byte-for-byte afterwards. That exercise found and fixed a real weakness: the claim-binding regressions originally used a catch-any-throw helper and would have passed on an incidental `TypeError`; the whole section now requires the stage's own typed `StageExecutionError`.

**Accepted limitations.** The stage is dormant: nothing calls it. Caption faithfulness, keyword relevance and timing usefulness are not verified, per the paragraphs above. Determinism is proven for the validators and the boundary **with an injected runner** — real model output is not deterministic and is not claimed to be. No stage executor in this repository has ever executed against a real model.

## Phase 0B — Content Intelligence runtime

**State:** foundation `MERGED` and `DEPLOYED`; the `strategy-concept`, `automotive-truth`, `hook-story-script`, and `production-direction` executors all **`MERGED`** and dormant, **none is `ENABLED`, established as `DEPLOYED`, or `PRODUCTION-VALIDATED`**, and none has production evidence; the `packaging-adaptation` executor **`IMPLEMENTED`** in a draft pull request and dormant, **not `MERGED`**; the remaining stage — `final-critic` — **not yet wired**.

**Next product slice: Phase 0B.6 — dormant `final-critic` stage executor.** It is named here as the cursor only; **its contract is not designed and no part of it is implemented in this record**. Deployment-authority work remains an independent track and must not be combined with it.
