...
**Unresolved follow-up / product cursor:** Phase 0B.4 — dormant `production-direction` stage executor, by name only. Its contract is not designed or implemented in this reconciliation. Deployment-authority work remains an independent track.

## Phase 0B — Content Intelligence runtime

**State:** foundation `MERGED` and `DEPLOYED`; `strategy-concept`, `automotive-truth`, and `hook-story-script` executors **`MERGED`** and dormant, **none is `ENABLED`, established as `DEPLOYED`, or `PRODUCTION-VALIDATED`**; the remaining three stages **registered but not wired**.
