...
**Automated validation (on the head that was pushed).** Build and typecheck clean. `npm run test:offline` **ALL PASS** on all nine suites — **1,722 checks** (1,712 before): posting 52, image 18, orchestrator 119, gate 56, API 51, render-identity one invariant pass, ownership/recovery 112, content-intelligence **1,219** (was 1,209), interval monitor 94. New checks `CM1`–`CM10`, appended after the last group: every writer loads `claim-boundaries` and every executed writer request carries it (`CM1`); stages 4 and 5 receive both blocks in order, byte-identical to the renderers (`CM2`, `CM3`); one renderer and one ceiling per block (`CM4`); no writer receives stage 2's assessment through the blocks, stages 4 and 5 receive it nowhere, and stage 3 only inside `TRUTH_OUTPUT` (`CM5`); stage 3's prompt no longer calls the lists advisory and binds them as narrow-only restrictions (`CM6`); stages 4 and 5's prompts do the same (`CM7`); the attribution rule is present and each writer prompt points to it (`CM8`); the skill stays fact-free (`CM9`); stages 4 and 5's ceilings count both blocks (`CM10`). Nine checks were re-specified, not loosened: `AT8`, `BC11`, `BL13` (three instruction assets), `AZ19`, `BI20`, `BS21` (the new asset lists), `BB10`, `BK9` (stage 2's assessment and restatements reach stages 4 and 5 nowhere; its caveats and forbidden claims only inside their own blocks), and `BU5`'s description. The mutation-harness, fake-run and remaining validation results are recorded in the pull request and in the handoff report for this change.
...
**Security and privacy implications.** Stages 4 and 5 are now shown two more pieces of stage 2's model prose, bounded by stage 2's validator and by derived block ceilings and framed as untrusted data. The prompts say the lists only narrow; the residual risk is a writer treating a caveat's wording as a fact, which no code detects — as no code detects any unfaithful paraphrase — and which the evidence lens reviews. Stage 2's assessment, restatements and wider whitelist stay withheld from stages 4 and 5 (`CM5`). The skill added to three instruction channels states no fact (`CM9`). No tool, provider, publishing, scheduling, approval, credential, model, effort, thinking or autonomy setting changed. The Phase-A approval gate and the live `brand-compliance-critic` are untouched. No customer data, credential or raw run output is committed.
...
- **The critic's evidence-lens prompt is now inaccurate in one sentence.** `agents/final-critic-evidence.md` still says `REQUIRED_CAVEATS` and `FORBIDDEN_CLAIMS` "are shown to you and to no writing stage after stage 3". The critic was out of this change's scope, so the sentence is unchanged. It does not change what the lens checks.
- **Stage 2's own prompt still calls `forbiddenClaims` "advisory"** and says it "is not a filter anything runs" (`agents/automotive-truth.md`). The second half remains true — no code filters on it. The prompt was not in this change's scope.
...
- An owner decision on the evidence-lens prompt sentence above (and, separately, the stage 2 prompt's "advisory" wording); each is prompt text, so each is its own reviewed change.
