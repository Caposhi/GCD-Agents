...
- **Evidence projection is bounded structurally, without discarding conflicts.** `maxProjectedRecords` is enforced by the real pack builder at 64 records. Exhaustive conflict detection has its own independently enforced 64-entry consumer boundary, because 64 same-subject/same-attribute facts produce all 2,016 pairs. The renderer, preview/context path, unusable-id accessors, and shared pre-model executor boundary refuse an over-bound pack intact; no pair is dropped and neither side is chosen as truth. The derived `EVIDENCE` ceiling counts 64 records and 64 conflict entries separately.
- **Evidence text is bounded compatibly in both languages.** Every textual allowance is enforced as both JavaScript code units/characters and UTF-8 bytes. `detail` is specifically bounded at 4,000 bytes of PostgreSQL's canonical `jsonb::text`; TypeScript computes a conservative upper bound for that same representation, so compact JavaScript JSON cannot pass while PostgreSQL rejects it. Relation notes use the owning 500 limit at every TypeScript consumption/read boundary. Migration 007 rejects NULL tag elements and uses a uniquely versioned, plain-`CREATE` helper so it cannot overwrite and later remove unrelated code. Its apply/enforce/rollback/reapply and collision-refusal paths run against disposable PostgreSQL 16 and 18. **Neither migration 007 nor its rollback has been applied to production**, and applying either is a separately authorized operator action.
- **Every contract-valid output fits its token budget.** Output fields are byte-bounded, and budgets price the escaping-aware serialized UTF-8 ceiling at a tokenizer-independent worst case of one token per byte — no ordinary-prose ratio and no provider call. The derived policy budgets are 40,000 / 79,000 / 73,000, replacing 4,000 / 3,000 / 2,000; all remain below the centrally recorded 128,000-token model output caps.
