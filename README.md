...
**Repository source and the live production release are separate facts and currently differ.** Production was last independently verified at `44d7336…` on 2026-08-28; the source lineage additionally carries the merged, dormant Phase 0B.1 through Phase 0B.5 executors, none established as deployed or production-validated, plus a draft, **unmerged** Phase 0B.6 `final-critic` executor (PR #52) — all six target stage executors now exist in source, but only the first five are merged, and all six remain dormant. Do not read exact current SHAs from this file — [Status](docs/STATUS.md) records dated production evidence and its freshness, and the exact current `main` is a Git lookup.

- `main` carries Phase 0A (PR #33), Phase 0D (PR #34), the documentation reconciliation (PR #35), the **worker ownership and recovery work (PR #36)**, roadmap-continuity governance (PR #37), **media publication normalization (PR #38)**, the **Phase 0B.0 content evidence and agent foundation (PR #40)**, and the dormant Phase 0B.1 through Phase 0B.5 executors (PR #42, PR #44, PR #46, PR #48, and PR #50). Migrations `001–006` are applied in production: `005_approval_integrity.sql` since Phase 0A, and `006_content_evidence.sql` since 2026-08-28. PR #40 is the only one of those pull requests that added a migration. A sixth executor, `final-critic`, is implemented on a draft, unmerged PR (#52) and is not part of `main`.
...
**`IMPLEMENTED` in source on draft PR #52 — not merged, not enabled, not established as deployed, and not production-validated.** This is the sixth and last of the six target stage executors; unlike Phase 0B.1 through 0B.5, it is **not yet part of `main`**. It stays on a draft, unmerged pull request pending independent review, per this record — merge, enablement, deployment, and production validation are all separately scoped future work, not part of this slice. **Production evidence: none.**
...
### Payload boundary: what this stage accepts, and what it does not

These two facts are true together and are stated together deliberately.

- **This stage's own `packagingOutputChars` ceiling is now safely above every Stage 5-valid serialized package.** It is a conservative, escaping-aware upper bound derived mechanically from Stage 5's exported maxima, so this stage's per-block bound can no longer be the reason a valid Stage 5 handoff is refused.
- **Stage 6 as a whole still does not accept every structurally valid Stage 5 handoff.** The *shared* assembled-payload boundary `MAX_PAYLOAD_CHARS` (120,000) in `stageExecution.ts` — merged infrastructure every stage uses, unchanged by this slice — applies to the whole framed payload. A maximal Stage 5 output exceeds it, and the refusal happens there, before any model call, with zero model requests.
- **There is no fixed packaging envelope to quote.** `SCRIPT_CLAIMS` and `PLATFORM_CLAIMS` have no finite structural maximum, because `EvidenceRecord.claim` is unbounded — non-empty in TypeScript, `length(btrim(claim)) > 0` in migration 006 — and a claim bound for several platforms is projected once per platform. The available packaging payload is `MAX_PAYLOAD_CHARS` minus the serialized sizes of the other five framed blocks, and with large enough valid evidence text that remainder is zero. Any figure quoted for it is an example for particular claim lengths, never a contract.
- **The same producer/consumer mismatch already exists in the merged stages** — Stage 1 → Stages 2/3, Stage 2 → Stage 3, Stage 3 → Stages 4/5/6, Stage 4 → Stages 5/6 — as does an output-side mismatch between the stage output contracts and their configured 2,000- and 3,000-token model budgets.
- **These are accepted dormant limitations, not production validation.** Every registry entry reports `executionEnabled: false`, nothing reaches an executor, and oversized input fails closed before any model call. A separately scoped **payload-contract reconciliation** — bounding evidence text in TypeScript and PostgreSQL after a read-only data audit, with explicit migration and rollback; deriving every producer/consumer guard and the shared boundary; reconciling output contracts with model `maxTokens`; adding derivation regressions; and considering narrower Stage 6 projections — is a **hard prerequisite to production wiring and to enabling any stage**. It is not a new phase number and is not started.

Details in [Architecture](docs/ARCHITECTURE.md), [Roadmap](docs/ROADMAP.md), and [Testing](docs/TESTING.md). Next step: a separately scoped production-wiring design/review — not started, and not part of this record.
