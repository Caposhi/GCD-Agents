...
The current reasoning flow is analytics, copywriter, image specification, hashtag/SEO/timing, platform formatter, and final critic under deterministic TypeScript control. Phase 0B.1 through Phase 0B.4 add the merged, dormant `strategy-concept`, `automotive-truth`, `hook-story-script`, and `production-direction` executors alongside it, and Phase 0B.5 adds a fifth, `packaging-adaptation`, which is **implemented in a draft pull request and not merged**. None is enabled, established as deployed, or production-validated, and none is reachable from a production path; every registry entry remains `executionEnabled: false`. It is not yet the target six-stage Content OS architecture and implements no empirical learning. Phase 0B.0 added the registry and evidence substrate those stages use; all five executors inject their reviewed prompt and skill assets into the instruction channel while factual reference assets stay out of it. None of this changes the production flow. See [Architecture](docs/ARCHITECTURE.md) and [Roadmap](docs/ROADMAP.md).
...
**`IMPLEMENTED` in a draft pull request — not merged, therefore not on `main`, not enabled, not deployed, not production-validated, and deliberately dormant.**
...
- **Deterministic platform policy is reused, not redeclared.** Instagram 8–15 hashtags and 2,200 characters, Facebook at most two, Google Business Profile zero hashtags and 1,500 characters — imported from `packageMap.ts`, the module that already enforces them on provider text. That module now exports those constants and names two previously inline literals; its behaviour is unchanged. **No competing value is defined anywhere.**
- **A naming divergence, recorded rather than hidden.** The provider-payload `Platform` union spells Google Business Profile `gbp`; this stage's closed enum spells it `google_business_profile`, because a stage-5 package is review metadata and must never be mistaken for a publishable payload. `PACKAGING_PLATFORM_PRODUCTION_ID` pins the one-to-one correspondence, and a test asserts it is total and injective so a rename or addition fails loudly.
- **A package is never a post, and a time is never a schedule.** Output is branded provisional, unverified, non-publishable **and non-executable**; caption wording is `captionVerified: false`, hashtag and local-keyword selection `selectionVerified: false`, and the timing recommendation both `timingVerified: false` and **`schedulable: false`**. The recommended time is a bounded `HH:MM ET` note carrying no date and no timestamp, so its shape refuses to become a scheduler instruction. The stage emits no provider payload, media URL or digest, destination/account/location id, CTA URL, hosted/QC flag, approval state, or API parameter.
- **What deterministic validation cannot do, stated plainly.** It checks structure, bounds, enums, platform coverage and order, ids, and used-claim membership. It **cannot** prove a caption faithfully preserves the script, that a shortening keeps meaning, that hashtags or local keywords are relevant or truthful, that a recommended time is useful, or that every factual implication was cited. **No language model in this pipeline proves any of those true.** A regression feeds a drifting caption, an unsupported local keyword, and a useless time; all **validate**, and none reaches an accessor result.
