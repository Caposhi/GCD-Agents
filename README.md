...
**Repository source and the live production release are separate facts and currently differ.** Production was last independently verified at `44d7336…` on 2026-08-28; the source lineage additionally carries the merged, dormant Phase 0B.1 through Phase 0B.6 executors and the merged payload-contract reconciliation (PR #54), none established as deployed or production-validated — **all six** target stage executors are now merged to `main`, and all six remain dormant with `executionEnabled: false`, reached by no production path. Do not read exact current SHAs from this file — [Status](docs/STATUS.md) records dated production evidence and its freshness, and the exact current `main` is a Git lookup.

- `main` carries Phase 0A (PR #33), Phase 0D (PR #34), the documentation reconciliation (PR #35), the **worker ownership and recovery work (PR #36)**, roadmap-continuity governance (PR #37), **media publication normalization (PR #38)**, the **Phase 0B.0 content evidence and agent foundation (PR #40)**, the dormant Phase 0B.1 through Phase 0B.6 executors (PR #42, PR #44, PR #46, PR #48, PR #50, and PR #52), and the **payload-contract reconciliation (PR #54)**. **As independently observed on 2026-08-28**, migrations `001–006` were applied in production — `005_approval_integrity.sql` since Phase 0A, `006_content_evidence.sql` as of that date. That is a dated observation, not a statement about now. Two of those pull requests added a migration: PR #40 added `006`, applied as of that observation; PR #54 added `007_evidence_bounds.sql`, which **exists in source**, and **whether it is now applied to production is `UNKNOWN` — not established in either direction**, no production database having been inspected since. Applying it requires its own authorization, audit, rollout, and verification.
...
- `state/migrations/`: forward-only PostgreSQL schema authority. **`001–006` were observed applied in production on 2026-08-28** — `006` as of that date. A dated observation; the current applied set is **`UNKNOWN` unless separately verified**, `007` included.
...
### Payload boundary: reconciled and `MERGED` (PR #54)

The mismatch this section used to record is closed in code and present on `main`. **The reconciliation is `MERGED` through PR #54** — base `a000557e4910d84e001d3be9078630542d21bb42`, reviewed head `0362e354bc942416c640e93d403c537421a184e3`, merge `0c13ab1af9c7ca796a1d48ed37207715a47166e4`, whose ordered parents are exactly that base then that head. It is **not established as deployed, not enabled, and not production-validated**, and no stage's `executionEnabled` changed. Repository evidence only: the merge changed repository state and authorizes no production wiring.
...
- **Evidence projection is bounded structurally, without discarding conflicts.** `maxProjectedRecords` is enforced by the real pack builder at 64 records, and every record is revalidated at pack construction, durable-row reconstruction, pack projection, and the shared pre-model boundary. Exhaustive conflict detection has its own independently enforced 64-entry consumer boundary, because 64 same-subject/same-attribute facts produce all 2,016 pairs. The renderer, preview/context path, unusable-id accessors, and shared pre-model executor boundary refuse an over-bound pack intact; no pair is dropped and neither side is chosen as truth. The derived `EVIDENCE` ceiling counts 64 records and 64 conflict entries separately.
- **Evidence text is bounded compatibly in both languages.** Every textual allowance is enforced as both JavaScript code units/characters and UTF-8 bytes. `detail` is specifically bounded at 4,000 bytes of PostgreSQL's canonical `jsonb::text`; TypeScript computes a conservative upper bound for that same representation, including the 327-byte signed `-5e-324` representation, so compact JavaScript JSON cannot pass while PostgreSQL rejects it. Relation notes use the owning 500 limit at every TypeScript consumption/read boundary. Migration 007 rejects NULL tag elements and uses a uniquely versioned, plain-`CREATE` helper so it cannot overwrite and later remove unrelated code. Its 208-check apply/enforce/rollback/reapply and collision-refusal paths run against disposable PostgreSQL 16.15 and 18.6. **Whether migration 007 or its rollback has been applied to production is `UNKNOWN` in either direction** — repository state is not production evidence, it establishes neither application nor non-application, the only dated observation is the 2026-08-28 read-only reading of `_migrations` at `001–006`, and the current state requires fresh read-only verification. Applying either file remains a separately authorized operator action.
- **Every contract-valid output fits its token budget.** Output fields are byte-bounded, and budgets price the escaping-aware serialized UTF-8 ceiling at a tokenizer-independent worst case of one token per byte — no ordinary-prose ratio and no provider call. The derived policy budgets are 40,000 / 79,000 / 73,000, replacing 4,000 / 3,000 / 2,000; all remain below the centrally recorded 128,000-token model output caps. Anthropic adaptive thinking shares `max_tokens`, so the centralized typed stage policy explicitly sends `thinking: { type: "disabled" }` and reserves the full derived ceiling for visible JSON; legacy `runAgent` callers and `runVision` retain their prior behavior.
...
- **The pack a stage is handed must mean what its shape claims.** `assertUsableEvidencePack` is the one runtime validator, called by the shared executor boundary, the renderer, the unusable-id set and the preview. It enforces each section's permitted evidence kinds and lifecycle, one section per record, `allowedFacts` restricted to active and fresh verified fact classes, conflicted/stale/inactive material barred from remaining usable, every conflict field (ids, subject, note, the closed `basis` enum, distinct and canonically ordered ids, referenced-record existence, pair uniqueness), the exact `counts` key set with honest integer values, the pack's `goal`, and `builtAt` itself. Before it, a valid hypothesis hand-placed in `allowedFacts` reached a stage runner and was cited as established fact.
- **Freshness is evaluated at the pack's own `builtAt`**, so the check is deterministic rather than a property of when it ran. The gap that leaves — a pack built before a fact lapses and consumed after — is bounded by validating `builtAt`, by every caller building the pack inside the operation that consumes it, and by an optional invocation-time check the preview passes.
- **A conflicted record is held, not vanished.** `EvidencePack` carries a `conflictedEvidence` section: the authoritative records behind every live conflict endpoint. The builder routes any record named in a live conflict there whatever its kind, and it is not a usable section, so nothing in it may be cited. A conflict's `aClaim`, `bClaim` and `subject` must be exact snapshots of the records it names, and every conflict string is bounded in both code units and UTF-8 bytes. The validator's first form rejected every legitimate conflict pack the builder produced — an inferred fact conflict left both endpoints in no section at all, and a declared conflict over non-fact evidence left both in usable ones — which refusal-only coverage could not see.
- **The streaming deadline is enforced, not assumed.** The Anthropic SDK's `timeout` option arms a timer around the underlying `fetch` and clears it once that resolves, which for a streaming request is when response *headers* arrive. It bounds request setup, not the response. The two bounds are now named apart — a one-minute `requestSetupTimeoutMs` and a budget-derived `streamDeadlineMs` — and the stage boundary arms its own timer for the second, aborts the stream if it expires, and clears the timer on both the success and failure paths.
