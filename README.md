...
- **Verdict consistency is checked structurally, not for correctness.** `verdict` cannot claim blocking findings are present with none marked blocking, or absent with one marked blocking, and a blocking finding always requires `requiresHumanReview: true`. This does not prove any finding is actually correct.
